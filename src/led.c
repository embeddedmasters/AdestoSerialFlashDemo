...
static const gpio_init_t led_info[] =
{
	{ LED0_PORT, LED0_PIN, gpioModePushPull, 0 },
	{ LED1_PORT, LED1_PIN, gpioModePushPull, 0 }
};

/***************************************************************************//**
 * @brief
 *   Initialize GPIO to drive LED
 *
 ******************************************************************************/
void led_init(void)
{
	gpio_init(led_info, sizeof(led_info)/sizeof(gpio_init_t));
}

/***************************************************************************//**
 * @brief
 *   Turn ON/OFF LED based on input Value
 *
 * @param[in] num
 * 		Input from LED table translate to GPIO Pin.
 * @param[in] Value
 * 		Value = 1 => LED ON, Value = 0 => LED OFF
 *
 ******************************************************************************/
void led_set(int num, int value)
{
	if (value)
		GPIO_PinOutSet(led_info[num].port, led_info[num].pin);
	else
		GPIO_PinOutClear(led_info[num].port, led_info[num].pin);
}
